...
fn get_ram_bank_size(rom: &[u8]) -> Result<usize, String> {
    let code = rom.get(0x149).copied().unwrap_or(0);
    match code {
        0 => Ok(0),
        1 => Ok(0),
        2 => Ok(1),
...
fn map_rom_into_bank(rom_image: &[u8]) -> Result<Vec<Vec<u8>>, String> {
    let banks: Vec<Vec<u8>> = rom_image
    ...
    if banks.len() != supposed_rom_bank_size {
        return Err(format!(
            "Inconsistent Rom Header : size must be : {}",
            supposed_rom_bank_size
        ));
    }
    Ok(banks)
}
...
fn map_n_ram_banks(
    ...
    Ok(if let Some(saved_ram) = saved_ram {
        let banks: Result<Vec<Vec<u8>>, String> = saved_ram
            .chunks(bank_size)
            .map(|chunk| {
                if chunk.len() != bank_size {
                    return Err(
                        "Chunk is not well sized .save was corrupted. You may delete it."
                            .to_string(),
                    );
                }
                Ok(chunk.to_vec())
        ...
        (0..ram_nb).map(|_| vec![0u8; bank_size]).collect()
    })
}

fn map_ram_banks(rom_image: &[u8], saved_ram: Option<Vec<u8>>) -> Result<Vec<Vec<u8>>, String> {
    let supposed_ram_bank_size = get_ram_bank_size(rom_image)?;
    println!("ram banks count {}", supposed_ram_bank_size);

    let ram_banks = map_n_ram_banks(supposed_ram_bank_size, saved_ram, RAM_BANK_SIZE)?;
...
impl Mbc for Mbc1 {
    ...
    fn new(rom_image: Vec<u8>, saved_ram: Option<Vec<u8>>) -> Result<Self, String> {
        ...
        if ram_banks.len() > 4 {
            Err("Supposed ram bank size can't be more than 4 in mbc1 cartridge.".to_string())
        } else {
            Ok(Mbc1 {
                banks,
        ...
        match addr {
            0x0000..0x4000 => {
                if self.mode_register {
                    ...
                    self.banks[(self.bank_register_2 << 5) as usize % self.banks.len()]
                        [addr as usize]
                }
            }
            0x4000..0x8000 => {
    ...
    fn write(&mut self, addr: u16, val: u8) {
        match addr {
            ...
            0x2000..0x4000 => {
                self.bank_register_1 = val & 0b11111;
            }
            0x4000..0x6000 => {
                self.bank_register_2 = val & 0b11;
            }
            0x6000..0x8000 => {
            ...
            0xA000..0xC000 => {
                ...
                if self.ram_gate_register {
                    let index = (self.mode_register as usize * self.bank_register_2 as usize)
                        % self.ram_banks.len();
                    self.ram_banks[index][addr as usize - 0xA000] = val
                }
            }
            _ => unreachable!(),
        }
...
impl Mbc for Mbc2 {
        ...
        Self: Sized,
    {
        ...
        let ram_banks = map_n_ram_banks(1, saved_ram, 0x200)?
            .into_iter()
            .next()
            .unwrap();
        Ok(Mbc2 {
...
impl Mbc for RomOnly {
    ...
    fn new(rom_image: Vec<u8>, saved_ram: Option<Vec<u8>>) -> Result<Self, String> {
        println!("New Romonly");
        let mut bank = vec![0u8; ONLY_ROM_SIZE];
    ...
    fn read(&self, addr: u16) -> u8 {
        match addr {
            0x0000..0x8000 => self.rom_banks[0][addr as usize],
            0xA000..0xC000 => {
                if self.ram_banks.is_empty() {
                    0xFF
                } else {
                    self.ram_banks[0][addr as usize - 0xA000]
                }
            }
            _ => unreachable!(),
        }
    ...
    fn write(&mut self, addr: u16, val: u8) {
        if (0xA000..0xC000).contains(&addr) && !self.ram_banks.is_empty() {
            self.ram_banks[0][addr as usize - 0xA000] = val;
        }
    }
...
impl Rtc {
    ...
    pub fn read_registers(&self, selector: u8) -> u8 {
        ...
        match selector {
            ...
            0x0C => {
                let mut dh = 0u8;
                if registers.day_counter & 0b1_0000_0000 != 0 {
                    dh |= 1;
                }
                if registers.halted {
                    dh |= 0b0100_0000;
                }
                if registers.day_carry {
                    dh |= 0b1000_0000;
                }
    ...
    fn read(&self, addr: u16) -> u8 {
        match addr {
            ...
            0xA000..0xC000 => {
                ...
                match selector {
                    0x00..0x08 if selector < self.ram_banks.len() => {
                        self.ram_banks[selector][(addr - 0xA000) as usize]
                    }
                    0x08..0x0D => self.rtc.read_registers(selector as u8),
                    _ => 0xFF,
                }
            }
            _ => unreachable!(),
    ...
    fn read(&self, addr: u16) -> u8 {
        match addr {
            ...
            0xA000..0xC000 => {
                ...
                self.ram_banks[(self.ram_bank_register as usize) % self.ram_banks.len()]

...
impl SelectionDevice {
    ...
    pub fn selection_view(mut self, ui: &mut egui::Ui, _frame: &mut eframe::Frame) -> AppState {
        if let Some(snap_path) = self.display(ui, _frame) {
    ...
    fn display(&mut self, ui: &mut egui::Ui, _frame: &mut eframe::Frame) -> Option<PathBuf> {
        ...
        let mut selected_saved_state = None;
